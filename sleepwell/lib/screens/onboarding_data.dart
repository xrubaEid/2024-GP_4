
 import 'onboardingInfo.dart';

class OnboardingData{
  List<OnboardingInfo> items = [
    OnboardingInfo(
        title: "Smart sleep planner",
        descriptions: "Wake up refreshed with optimized wake-up times based on your sleep cycles. Say goodbye to tired mornings.",
        image: "assets/sleepicon.png"),

    OnboardingInfo(
        title: "flexible alarm",
        descriptions: "Customize your wake-up with the various alarm options to providing a tailored and enjoyable wake-up feature",
        image: "assets/alarmicon2.png"),

    OnboardingInfo(
        title: "enhance your kid's sleep",
        descriptions: "Track and monitor your family's sleep patterns! Add your kids as beneficiaries to ensure they get the rest they need.",
        image: "assets/USERICON.png"),


  ];
 }


